...
geometry_msgs::Pose Door::getSystemPose(int index)
{
    ...
    this->transform2pose(this->puerta.at(index), p);
    return p;
}
...
float Door::distancia_euclidea(geometry_msgs::Point p, geometry_msgs::Point q)
{
    return sqrt(pow(p.x-q.x, 2) + pow(p.y-q.y, 2) + pow(p.z-q.z, 2));
}

void Door::transform2pose(Affine3d t, geometry_msgs::Pose &pose)
{
